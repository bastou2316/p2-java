...
public class JPanelResultStep extends JPanel
	...
	private void updateDisplay()
		...
		if (currentStep != 0)
			{
				//jPanelPreviousStep.setVisible(true);
				textPrevious.setText(controllerEquation.getPreviousMatrix().toString());
			}
		else
			{
			textPrevious.setText("None");
			}
		if (!controllerEquation.isFinalStep())
			{
	...
	private void geometry()
		...
		textPrevious = new JTextArea();
		textPrevious.setEditable(false);
		textPrevious.setText(controllerEquation.getMatrix(0).toString());
		//textMatrix.setColumns(controllerEquation.getEquation().getMatrixNumberVariable());
		textPrevious.setRows(controllerEquation.getEquation().getMatrixNumberEquation());
		actualTextSizePrev = textPrevious.getFont().getSize();
		...
		JScrollPane scrollPanePrevious = new JScrollPane(textPrevious, ScrollPaneConstants.VERTICAL_SCROLLBAR_ALWAYS, ScrollPaneConstants.HORIZONTAL_SCROLLBAR_ALWAYS);
		...
		splitPaneV.add(jPanelPreviousStep, JSplitPane.TOP);
		splitPaneV.add(jPanelMatrix, JSplitPane.BOTTOM);
		...
		splitPane.add(splitPaneV, JSplitPane.RIGHT);
	...
	private void control()
		...
		textPrevious.addMouseWheelListener(new MouseWheelListener()
			{

				@Override
				public void mouseWheelMoved(MouseWheelEvent e)
					{
					if (actualTextSizePrev > 0)
						{
						actualTextSizePrev += e.getWheelRotation();
						//System.out.println(actualTextSize);
						textPrevious.setFont(new Font("Sans-Serif", Font.PLAIN, actualTextSizePrev));
						}
					}
			});
		}
	...
	private JTextArea textMatrix, textPrevious;
	private JPanel jPanelPreviousStep;

	private int actualTextSize, actualTextSizePrev;
