...
//Code venant du projet JAMA: A Java Matrix Package
...
public class QRDecomposition implements java.io.Serializable
	...
	*/
	...
	public QRDecomposition(Matrix A)
		...
		for(int k = 0; k < n; k++)
			...
			for(int i = k; i < m; i++)
				{
				nrm = MathTools.hypot(nrm, QR[i][k]);
				}
	...
	*/
	...
	public Matrix solve(Matrix B)
		{
		if (B.rowCount() != m) { throw new IllegalArgumentException("Matrix row dimensions must agree."); }
		if (!this.isFullRank()) { throw new RuntimeException("Matrix is rank deficient."); }

		// Copy right hand side
		int nx = B.columnCount();
		double[][] X = B.getValuesCopy();
