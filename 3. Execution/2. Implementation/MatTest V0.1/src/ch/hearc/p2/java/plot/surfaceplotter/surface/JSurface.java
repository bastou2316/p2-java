...
import ch.hearc.p2.java.plot.surfaceplotter.DefaultSurfaceModel;
...
public class JSurface extends javax.swing.JComponent {
	...
	public void setModel(SurfaceModel model) {
		...
		surfaceVertex = new SurfaceVertex[3][];
...
//	}
	...
	private final SurfaceVertex values3[] = new SurfaceVertex[4];
	...
	private final void plotArea(int start_lx, int start_ly, int end_lx, int end_ly, int sx, int sy) {
		...
		while (ly != end_ly) {
			...
			if (plotfunc2) {
				 ...
				 * Spécial
				 */
				values3[1] = surfaceVertex[2][lx + ly];
				values3[2] = surfaceVertex[2][lx + ly + sy];
			}
			...
			while (lx != end_lx) {
				...
				if (plotfunc2) {
					 ...
					 */
					values3[0] = values3[1];
					values3[1] = surfaceVertex[2][lx + sx + ly];
					values3[3] = values3[2];
					values3[2] = surfaceVertex[2][lx + sx + ly + sy];
				}
				if (!plotboth) {
					if (plotfunc1) {
						curve = 1;
					...
					if (plottable(values1)) {
						if (plottable(values2)) {
							if(plottable(values3)) {
								...
								splitPlotPlane(values1, values3);
								splitPlotPlane(values2, values3);
							}
							else{
								splitPlotPlane(values1, values2);
							}
						}							
						else if (plottable(values3)) {
							splitPlotPlane(values1, values3);
						}
						else {
							curve = 1;
